...
// getAllTasks() and addTaskCard(task, index)
// get all the tasks from localStorage and add it to taskbox -> .card .todo, .card .pending, .card .completed divs using css form template. 
Kanban.getAllTasks().forEach((tasks, index) => {
    tasks.forEach(task => {
        // 1st simple method:
        // taskbox[index].innerHTML += `<form class="card" draggable="true" data-id="${task.taskId}">
        //                                   <input value="${task.content}" type="text" name="task" autocomplete="off" disabled="disabled"/>
        //                                   <div>
        //                                       <span class="task-id">#${task.taskId}</span>
        //                                       <span>
...
// inerstTask() - adding form to columns
